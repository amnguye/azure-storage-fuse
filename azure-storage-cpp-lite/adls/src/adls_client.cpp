...
namespace microsoft_azure { namespace storage_adls {
    ...
    {
        return blob_client_adaptor<void>(std::bind(&microsoft_azure::storage::blob_client::create_container, m_blob_client, filesystem));
    }

    void adls_client::delete_filesystem(const std::string& filesystem)
    {
        return blob_client_adaptor<void>(std::bind(&microsoft_azure::storage::blob_client::delete_container, m_blob_client, filesystem));
    }
    ...
    bool adls_client::filesystem_exists(const std::string& filesystem)
    {
        ...
        try
        {
            auto container_properties = blob_client_adaptor<microsoft_azure::storage::container_property>(std::bind(&microsoft_azure::storage::blob_client::get_container_properties, m_blob_client, filesystem));
    ...
    {
        return blob_client_adaptor<void>(std::bind(&microsoft_azure::storage::blob_client::set_container_metadata, m_blob_client, filesystem, properties));
    }
    ...
    {
        auto containers_segment = blob_client_adaptor<microsoft_azure::storage::list_containers_response>(std::bind(&microsoft_azure::storage::blob_client::list_containers, m_blob_client, prefix, continuation_token, max_results, false));
    ...
    bool adls_client::directory_exists(const std::string& filesystem, const std::string& directory)
    {
        ...
        try
        {
            auto blob_properties = blob_client_adaptor<microsoft_azure::storage::blob_property>(std::bind(&microsoft_azure::storage::blob_client::get_blob_properties, m_blob_client, filesystem, directory));
    ...
    {
        return blob_client_adaptor<void>(std::bind(&microsoft_azure::storage::blob_client::set_blob_metadata, m_blob_client, filesystem, directory, properties));
    }
    ...
    list_paths_result adls_client::list_paths_segmented(const std::string& filesystem, const std::string& directory, bool recursive, const std::string& continuation_token, const int max_results)
    {
        ...
        auto async_func = std::bind(&microsoft_azure::storage::async_executor<std::vector<list_paths_item>>::submit, m_account, request, http, m_context);
    ...
    void adls_client::append_data_from_stream(const std::string& filesystem, const std::string& file, uint64_t offset, std::istream& in_stream, uint64_t stream_len)
    {
        ...
        http->set_input_stream(microsoft_azure::storage::storage_istream(in_stream));
    ...
    {
        return blob_client_adaptor<void>(std::bind(&microsoft_azure::storage::blob_client::delete_blob, m_blob_client, filesystem, file, false));
    }
    ...
    bool adls_client::file_exists(const std::string& filesystem, const std::string& file)
    {
        ...
        try
        {
            auto blob_properties = blob_client_adaptor<microsoft_azure::storage::blob_property>(std::bind(&microsoft_azure::storage::blob_client::get_blob_properties, m_blob_client, filesystem, file));
            blob_exists = blob_properties.valid();
        }
        catch (const storage_exception& e)
        {
    ...
    {
        return blob_client_adaptor<void>(std::bind(&microsoft_azure::storage::blob_client::set_blob_metadata, m_blob_client, filesystem, file, properties));
    }
    ...
    dfs_properties adls_client::get_dfs_path_properties(const std::string &filesystem, const std::string &path) {
        auto http = m_blob_client->client()->get_handle();
        ...
        if (success())
        {
            ...
            props.resource_type = http->get_response_header(constants::header_ms_resource_type);
            ...
            for(char c : http->get_response_header(constants::header_ms_properties)) {
                if(propName.empty()) {
                    if (c == '=') {
